                ...
                <div class="card table-card">
                    <div class="card-header">
                            ...
                            <div class="btn-group mb-2 mr-2">
                                ...
                                <div class="dropdown-menu">
                                    <form class="form-inline c-data-table__filters" data-target="#files">
                                        ...
                                        <a class="dropdown-item  download_all_files" id="<?php echo get_the_ID() ?>" href="#!">Download All</a>
                        ...
                        <div class="btn-group mb-2 mr-2">
                            ...
                            ?>
                                <li id="approve-design" style="background-color:#5cd165; margin-left: 10px; list-style:none">
                                    <a href="#" id="<?php echo get_the_ID(); ?>" class="approve_design_btn btn btn-outline-success" style="color: #fff">
                                        <?php
                            ...
                            ?>
                                ...
                                <li id="request_for_approve_design_btn" style="background-color:#5cd165; margin-left: 10px; list-style:none">
                                    <a href="#" id="<?php echo get_the_ID(); ?>" class="request_for_approve_design_btn  btn btn-outline-success" style="color: #fff">
                                        <i class="fa fa-check" style="color:#fff" ;=""></i>
                                        Request Approval
                                    </a>
                        ...
                        <div class="btn-group mb-2 mr-2">
                            <?php
                            if (in_array('upstream_client_user', $user->roles)) { ?>
                                <a style="display:none" class="btn btn-outline-primary" data-toggle="up-modal" data-target="#modal-file" data-form-type="add" data-modal-title="New: Design File">Add Revision</a>
                            <?php } else {
                            ?>
                                <a class="btn btn-outline-primary" data-toggle="modal" data-target="#modal-file" data-form-type="add" data-modal-title="New: Project Files">Add Project Files </a>
                            <?php } ?>
                                                ...
                                                <tr>
                                                    <td>
                                                        <div class="chk-option">
                                                            <label class="check-task custom-control custom-checkbox d-flex justify-content-center done-task">
                                                                <input data-file="<?php echo $pdata['file'] ? $pdata['file'] : 'no_file';  ?>" type="checkbox" class="custom-control-input" data-item-file="<?php echo $file_path ? $file_path : 'no_file' ?>">
                                                                <span class="custom-control-label"></span>
                                                            </label>
                                                        </div>
                                                        <div class="d-inline-block align-middle">
                                                            <div class="d-inline-block">
                                                                <h6 class="revision-file-name">.<?php printf('%s', $file_path); ?> File</h6>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    ...
                                                    if (in_array('upstream_client_user', $user->roles)) {
                                                    ?>
                                                        <td data-file_id="<?php echo $pdata['file_id'] ?>" data-filename="<?php echo $file_path ?>" data-item-title="<?php echo $title ?>" data-item-id="<?php echo $item_id ?>" data-project-type="<?php echo $project_type ?>" class="open-revision-popup" id="<?php echo $pdata['id']; ?>" data-toggle="modal" data-target="#modal-revision">Add Revision Comment</a>
                                                        </td>
                                                    <?php }
                                                    ?>

                                                    <td data-column="file" data-value="<?php printf('%s', $pdata['file']); ?>" data-type="file" data-order="<?php printf('%s', $pdata['file']); ?>">
                                                        <a href="<?php printf('%s', $pdata['file']); ?>" target="_blank">Download .<?php printf('%s', $file_path); ?> file</a>
                                                    </td>
                ...
                success: function(response) {
                    if (response.error) {
                        ...
                        if (!response.success) {
                            ...
                            let attachment_data = $('#upload_revision_in_reply').attr('data-uploading_info')
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    if (res.response == 'missing_parameter') {
                        alert('One or more parameter is missing')
                    }
                    if (res.response == 'file_exists') {
                        alert('This File already uploaded for this revision comment')
                    }
                    if (res.response == 'failed') {
                        alert('Failed to upload file. There is something error')
                    }
                    if (res.response == 'success') {
                        if (res.msg == 'mail_sent') {
                            alert('File Uploaded & Mail Sent Successfully')
                        } else {
                            alert('File Uploaded Successfully But Mail Could Not Be Sent')
                        }
                        location.reload();
                    }
                },
